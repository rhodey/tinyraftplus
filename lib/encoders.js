...
class XxHashEncoder {
  ...
  async decodeLock(buf, o) {
    ...
    if (!hash.equals(data)) { throw new Error(`${name} lock decode error`) }
  ...
  async decodeMeta(buf, o) {
    ...
    data = await xxhash64(data)
    data = Buffer.from(data, 'hex')
    if (!hash.equals(data)) { throw new Error(`${name} meta decode error`) }
  ...
  async _decodeBody(buf) {
    ...
    data = Buffer.from(data, 'hex')
    if (!hash.equals(data)) { throw new Error(`${name} body decode error`) }
    return buf.slice(16)
  }
}
