...
class EncryptingEncoder {
  ...
  nonce(n=1) {
    const { sodium } = this
    const len = n * sodium.crypto_secretbox_NONCEBYTES
    const nonce = sodium.randombytes_buf(len)
    return Buffer.from(nonce)
  }

  async hashPrev(log, prev, buf, off) {
    const { path: name } = log
    if (prev === null || prev === undefined) { prev = Buffer.from('null', 'utf8') }
    const hash = await xxhash128(prev).catch((err) => {throw new Error(`${name} prev encode error`)})
    writeHash128(hash, buf, off)
  }

  async encode(log, seq, prev, body, nonce=null) {
    const meta = Buffer.allocUnsafe(24)
    meta.writeBigInt64BE(seq, 0)
    await this.hashPrev(log, prev, meta, 8)
