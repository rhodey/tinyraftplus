...
class Encoder {
  ...
  async encodeLock(log, buf, o, olen, llen) {
  ...
  async encodeMultiLock(log, buf, o, seq) {
    buf.writeBigInt64BE(seq, o)
  }

  async decodeMultiLock(log, buf, o) {
...
class XxHashEncoder {
  ...
  async encodeMultiLock(log, buf, o, seq) {
    ...
    const str = await xxhash64(data).catch((err) => {throw new Error(`${name} multi lock encode error`)})
    writeHash64(str, buf, o)
  }
  ...
  async decodeMultiLock(log, buf, o) {
    ...
    data = await xxhash64(data).catch((err) => {throw new Error(`${name} multi lock decode error`)})
    data = Buffer.from(data, 'hex')
    if (!hash.equals(data)) { throw new Error(`${name} multi lock hash incorrect`) }
