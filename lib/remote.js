  ...
  async open() {
    ...
    const clientClose = (sock) => {
      this._disconnect(sock)
        .catch((err) => this.emit('error', new Error(`close logs on disconnect error ${err.message}`)))
    }
...
class TcpLogClient extends EventEmitter {
  ...
  _send(cmd) {
    const okSock = ['new', 'open', 'del']
    return new Promise((res, rej) => {
      if (!this.sock && okSock.includes(cmd.type)) { return rej(new Error(`${this.name} is not connected`)) }
      if (!this.isOpen && !okSock.includes(cmd.type)) { return rej(new Error(`${this.name} is not open`)) }
      cmd.cid = randID()
  ...
  async open() {
    ...
    await this._send({ type: 'open' }).then((msg) => {
      this.seq = msg.seq
      this.head = msg.head
      this._open = true
    })
