...
// todo: test append batch
// todo: append data not too big
class MultiFsLog {
  ...
  async _readLock(fh) {
    ...
    return enc.decodeMultiLock(this, buf, 0)
  }
  ...
  async _writeLock(arg1, arg2) {
    ...
    await enc.encodeMultiLock(this, buf, 0, seq)
    const write = async () => {
  ...
  async start() {
    ...
    let [_, logs] = await Promise.all(works)
    ...
    const empty = logs.filter((log) => log.seq < 0)
    await Promise.all(empty.map((log) => log.stop()))
    await Promise.all(empty.map((log) => log.del()))
    logs = logs.filter((log) => log.open())
  ...
  async truncate(seq='-1') {
    ...
    for (const log of this.logs) {
      ...
      await log.truncate(rel)
      sum += BigInt(log.seq) + 1n
      this.head = log.head ?? this.head
    }
    const empty = this.logs.filter((log) => log.seq < 0)
    await Promise.all(empty.map((log) => log.stop()))
    await this._writeLock(null)
      .then(() => sync(this.fhlock))
    this.logs = this.logs.filter((log) => log.open())
