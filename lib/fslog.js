...
// todo: accept BigInt
const truncate = (fh, offset) => {
  console.log('TRUNC', fh.name, offset)
  return fh.truncate(Number(offset)).catch((err) => {throw new Error(`${fh.name} truncate error - ${err.message}`)})
}
...
class FsLog {
    ...
    return this._readOffset().then((ok) => {
      ...
      return this._readHead(off).then((head) => {
        if (hlen !== BigInt(head.byteLength)) {
          throw new Error(`${this.path} offset file says hlen = ${hlen} but got ${head.byteLength}`)
        }
        this.offset = off
        this.olen = olen
        this.hlen = hlen
  ...
  async truncate(seq='-1') {
    ...
    works.push(sync(this.fhoff))
    await Promise.all(works)
    this.rollForwardCb(seq.toString())
    await this._writeLock(null, null)
      .then(() => sync(this.fhlock))
