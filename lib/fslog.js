...
class FsLog {
  ...
  async _handleRollback() {
    ...
    try {
      ...
      // rollback
      let works = []
      let { olen, hoff, hlen } = lock
      olen = max(olen, 0n)
      works.push(truncate(fhoff, olen))
      hoff = max(hoff, 0n)
      hlen = max(hlen, 0n)
      works.push(truncate(fhlog, hoff + hlen))
  ...
  async append(data, seq=null) {
    ...
    // state to lock
    await this._writeLock(this.olen, this.offset, this.hlen)
