...
class FsLog {
  ...
  async txn(_seq=null) {
    ...
    const count = this._txn_c
    const callOrThrow = (fn) => {
      if (!cb) { throw new Error(`${path} txn already commit or abort`) }
      if (count !== this._txn_c) { throw new Error(`${path} was closed`) }
      return fn()
    }
    const api = this._txn.catch(noop).then(() => {
      ...
      return {
        ...
        _commit: () => callOrThrow(() => this._commit().then(() => cb)),
        commit: () => callOrThrow(() => this._commit().then(cb).then(() => cb = null)),
        abort: () => callOrThrow(() => this._abort().then(cb).then(() => cb = null)),
      }
