...
class FsLog {
  ...
  async _writeOffset(fh, offset) {
    ...
    const truncate = () => fh.truncate(0).catch((err) => {throw new Error(`truncate ${name} error - ${err.message}`)})
    return buf ? write() : truncate()
  }
  ...
  async _handleLock() {
    this.fhlock = await openOrCreate(`${this.path}.lock`, 'r+')
    const offset = await this._readOffset(this.fhlock)
