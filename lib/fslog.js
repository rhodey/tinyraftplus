...
class FsLog {
  ...
  async _appendOffset(arg1, arg2, arg3, arg4) {
    ...
    const write = async () => {
      ...
      console.log('WROTE OFF', seq, off, len, buf.byteLength)
  ...
  async _readLock(fh) {
    ...
    if (buf.byteLength !== 32) { throw new Error(`${fh.name} read len != 32 error`) }
    const olen = buf.readBigInt64BE(0)
    const hseq = buf.readBigInt64BE(8)
    const hoff = buf.readBigInt64BE(16)
    const hlen = buf.readBigInt64BE(24)
    return { olen, hseq, hoff, hlen }
  }

  async _writeLock(arg1, arg2, arg3, arg4, arg5) {
    const fh = arg5 !== undefined ? arg1 : this.fhlock
    const olen = arg5 !== undefined ? arg2 : arg1
    const hseq = arg5 !== undefined ? arg3 : arg2
    const hoff = arg5 !== undefined ? arg4 : arg3
    const hlen = arg5 !== undefined ? arg5 : arg4
    if (olen === null) { return truncate(fh, 0) }
    const buf = Buffer.alloc(32)
    buf.writeBigInt64BE(olen)
    buf.writeBigInt64BE(hseq, 8)
    buf.writeBigInt64BE(hoff, 16)
    buf.writeBigInt64BE(hlen, 24)
    const write = async () => {
  ...
  async _handleRollback() {
    ...
    try {
      ...
      // rollback
      const fhlog = await openOrCreate(`${this.path}.log`, 'r+')
      handles.push(fhlog)
      const fhoff = await openOrCreate(`${this.path}.off`, 'r+')
      handles.push(fhoff)

      // rollback
      let works = []
      const { olen, hseq, hoff, hlen } = lock
      console.log('READ LOCK', olen, hseq, hoff, hlen)
      ...
      // reset lock
      await this._writeLock(fhlock, null, null, null, null)
        .then(() => sync(fhlock))
        .then(() => sync(this.dir))
    ...
    return this._readOffset().then((ok) => {
      ...
      return this._readHead(off).then((head) => {
        if (hlen !== BigInt(head.byteLength)) {
          throw new Error(`${this.path} offset file says hlen = ${len} but got ${head.byteLength}`)
        }
        this.offset = off
        this.olen = olen
        this.hlen = hlen
  ...
  async append(data, seq=null) {
    ...
    // state to lock
    await this._writeLock(this.olen, BigInt(this.seq), this.offset, this.hlen)
    ...
    // reset lock
    await this._writeLock(null, null, null, null)
      .then(() => sync(this.fhlock))

    // ok
    this.offset = offset
    this.olen = max(this.olen, 0n) + olen
    this.hlen = len
    this.head = data
    this.seq = seq
    return { seq, data }
...
// todo: close if attempt truncate while iter
class FsIterator {
