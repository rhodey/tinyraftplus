...
class FsIterator {
  ...
  async *lazy() {
    ...
    while (next <= last) {
      ...
      const buf = await log._readOffsets(next, iterStepSize)
      if (!this._open) { throw new Error(`${this.name} is not open`) }

      for (let b = 0; b < buf.byteLength; b += 16) {
        const seq = buf.readBigInt64BE(0)
        const off = buf.readBigInt64BE(8)
      ...
      const firstOffset = works[0]

      const endOf

      const lastOffset = works[works.length - 1]
