...
class FsLog {
  ...
  async _handleResetLock() {
    ...
    try {
      ...
      // truncate fhoff and fhlog using fhlock
  ...
  // todo: prevent multi open
  async open(_again=false) {
    if (this._open && !_again) { return }
    const { path } = this

    const works = []
    await this._handleResetLock()
    !_again && works.push(openOrCreate(`${path}.lock`, 'r+').then((fh) => this.fhlock = fh))
    !_again && works.push(openOrCreate(`${path}.off`, 'a+').then((fh) => this.fhoff = fh))
    !_again && works.push(openOrCreate(`${path}.log`, 'a+').then((fh) => this.fhlog = fh))
