...
class FsLog {
  ...
  async _appendOffsets(arg1, arg2) {
    const fh = arg2 !== undefined ? arg1 : this.fhoff
    const offs = arg2 !== undefined ? arg2 : arg1
    let o = 0
    const buf = Buffer.alloc(24 * offs.length)
    offs.forEach((off) => {
      buf.writeBigInt64BE(o + off.seq)
      buf.writeBigInt64BE(o + off.off, 8)
      buf.writeBigInt64BE(o + off.len, 16)
      o += 24
    })
    const write = async () => {
      const { bytesWritten } = await fh.write(buf).catch((err) => {throw new Error(`${fh.name} write error - ${err.message}`)})
  ...
  _appendLog(arg1, arg2) {
    ...
    if (!Array.isArray(data)) {
      ...
      len = data.map((d) => BigInt(data.byteLength))
      total = Number(len.reduce((acc, bi) => acc + bi, 0n))
    }
    const write = async () => {
      const { bytesWritten } = await fh.write(data).catch((err) => {throw new Error(`${fh.name} write error - ${err.message}`)})
      if (bytesWritten !== total) { throw new Error(`${fh.name} write len error`) }
    }
    return { len, write }
  }
  ...
  async appendBatch(data, seq=null) {
    ...
    // write data
    let works = []
    const { len: lens, write } = this._appendLog(data)
    works.push(write())
