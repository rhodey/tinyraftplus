...
class FsLog {
  ...
  async _readOffset(fh) {
    ...
    if (!str) { return null }
    return parseStrInt(str, new Error(`${fh.name} parse error - ${str}`))
  }
  ...
  async _writeOffset(fh, offset) {
    ...
    return buf ? write() : truncate(fh, 0)
  }
  ...
  async _readHead(fh, offset) {
    offset = (typeof fh === 'number') ? fh : offset
    fh = (typeof fh === 'number') ? this.fhlog : fh
  ...
  async _handleRollback() {
    ...
    try {
      ...
      handles.push(fhlock)
      let offset = await this._readOffset(fhlock)
      if (fhlock.isNew) {
        await sync(fhlock)
        await sync(this.dir)
      }
      ...
      // rollback
      let works = []
      works.push(openOrCreate(`${this.path}.log`, 'r+'))
      ...
      // rollback
      const { obj, len } = await this._readHead(fhlog, offset)
      works = []
      const start = Math.max(offset, 0)
      works.push(truncate(fhlog, start + len))
      works.push(this._writeOffset(fhoff, offset))
      ...
      // reset lock
      await this._writeOffset(fhlock, null)
        .then(() => sync(fhlock))
  ...
  async start() {
    ...
    return this._readOffset(this.fhoff).then((offset) => {
  ...
  async append(data, seq=null) {
    ...
    // save state to lock
    await this._writeOffset(this.fhlock, this.offset)
    ...
    // reset lock
    await this._writeOffset(this.fhlock, null)
      .then(() => sync(this.fhlock))
