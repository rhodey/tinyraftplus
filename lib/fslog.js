...
class FsLog {
  ...
  async _handleRollback() {
    ...
    try {
      ...
      // todo: dont think this guarantees close
  ...
  async start() {
    ...
    return this._readOffset().then((ok) => {
      ...
      const { seq, off, len } = ok
      return this._readHead(off).then((head) => {
        if (len !== BigInt(head.byteLength)) {
          throw new Error(`${this.path} offset file says head len = ${len} but got ${head.byteLength}`)
        }
        this.offset = off
        this.head = head
        this.hlen = len
...
class FsIterator {
  ...
  // todo: throw error if return before got what we expected
  async *lazy() {
