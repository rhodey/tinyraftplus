...
class FsLog {
  ...
  async _readOffset(fh) {
    ...
    let str = await fh.readFile('utf8').catch((err) => {throw new Error(`read ${name} error - ${err.message}`)})
  ...
  async _writeOffset(fh, offset) {
    ...
    const { bytesWritten } = await fh.write(buf).catch((err) => {throw new Error(`write ${name} error - ${err.message}`)})
