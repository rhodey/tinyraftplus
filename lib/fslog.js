...
// todo: BigInt in fs all
// todo: check allocUnsafe
...
const openOrCreate = async (path, flag, again=true) => {
  ...
  dir.pop()
  await mkdirp(dir.join('/')).catch((err) => { throw new Error(`${dir} mkdirp error - ${err.message}`) })
...
class FsLog {
  ...
  // restore log if needed and reset lock
  ...
  open(_again=false) {
    if (this._open && !_again) { return ready }
    if (this.opening && !_again) { return this.opening }
    const opening = new Promise(async (res, rej) => {
      try {

        const works = []
        await this._handleResetLock()
        !_again && works.push(openOrCreate(`${this.path}.lock`, 'r+').then((fh) => this.fhlock = fh))
        !_again && works.push(openOrCreate(`${this.path}.off`, 'a+').then((fh) => this.fhoff = fh))
        !_again && works.push(openOrCreate(`${this.path}.log`, 'a+').then((fh) => this.fhlog = fh))

        !_again && await Promise.all(works).then((arr) => {
    ...
    !_again && (this.opening = opening)
    return opening
  }

  async close() {
    if (!this._open) { return }
    // counter to allow close transactions
    this._txn_c++
    this._txn = ready
    const works = []
  ...
  async txn(_seq=null) {
    const { path } = this
    if (!this._open) { throw new Error(`${path} is not open`) }
    let cb = null
    const delay = new Promise((res, _) => cb = res)
    const c = this._txn_c
    const callOrThrow = async (fn) => {
      if (!cb) { throw new Error(`${path} txn already commit or abort`) }
      if (c !== this._txn_c) { throw new Error(`${path} was closed`) }
      return fn()
    }
    const callOnce = async (fn) => {
      return callOrThrow(fn).then((ok) => {
        cb(); cb = null
        return ok
      })
    }
    const api = this._txn.catch(noop).then(() => {
      callOrThrow(noop)
      _seq = _seq === null ? this.seq : _seq
      return this._writeLock(_seq)
      ...
      return {
        ...
        _commit: () => callOrThrow(() => this._commit(c).then(() => cb)),
        commit: () => callOnce(() => this._commit(c)),
        abort: () => callOnce(() => this._abort(c)),
      }
  ...
  _throwIfClosed(c) {
    if (c !== this._txn_c) { throw new Error(`${this.path} was closed`) }
  }

  async _commit(c) {
    if (!this._open) { throw new Error(`${this.path} is not open`) }
    const works = []
    this._throwIfClosed(c)
  ...
  // truncate is fail forward / cannot be supported by txn api
  // txn = true = return txn to caller to share in same
  async truncate(seq=-1n, txn=false) {
    const c = this._txn_c
    const parseArgs = () => {
      const { path } = this
      if (!this._open) { throw new Error(`${path} is not open`) }
    ...
    const _txn = await this.txn(seq)
    const empty = parseArgs() === true
    if (empty && txn) {
      return _txn
    } else if (empty) {
      await _txn.commit()
