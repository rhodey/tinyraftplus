...
class FsLog {
  ...
  async txn(_seq=null) {
    ...
    const api = this._txn.catch(noop).then(() => {
      ...
      const dup = Promise.resolve()
      return {
        append: (data, seq) => this.append(data, seq, true),
        appendBatch: (data, seq) => this.appendBatch(data, seq, true),
        _commit: () => this._commit().then(() => cb),
        commit: () => cb ? this._commit().then(cb).then(() => cb = null) : dup,
        abort: () => cb ? this._abort().then(cb).then(() => cb = null) : dup,
      }
    })
    this._txn = this._txn.catch(noop).then(() => delay)
    api.catch(noop).then(() => delay).then(cb)
    return api
  }
