...
// todo: accept BigInt
const truncate = (fh, offset) => fh.truncate(Number(offset)).catch((err) => {throw new Error(`${fh.name} truncate error - ${err.message}`)})
...
class FsLog {
  ...
  async _appendOffset(arg1, arg2, arg3, arg4) {
    const fh = arg4 !== undefined ? arg1 : this.fhoff
    const seq = arg4 !== undefined ? arg2 : arg1
    const off = arg4 !== undefined ? arg3 : arg2
    const len = arg4 !== undefined ? arg4 : arg3
    const buf = Buffer.alloc(24)
  ...
  async _readHead(arg1, arg2) {
    ...
    const { bytesRead } = await read(fh, buf, 0, len, offset).catch((err) => {throw new Error(`${fh.name} read head error - ${err.message}`)})
    if (bytesRead !== len) { throw new Error(`${fh.name} read head len error`) }
    return buf
  }

  async _readLog(offset, len) {
    const { fhlog: fh } = this
    len = Number(len)
    const buf = Buffer.alloc(len)
    const { bytesRead } = await read(fh, buf, 0, len, offset).catch((err) => {throw new Error(`${fh.name} read log error - ${err.message}`)})
    if (bytesRead !== len) { throw new Error(`${fh.name} read log len error`) }
    return buf
  }
  ...
  async _handleRollback() {
    ...
    try {
      ...
      works.push(open(`${this.path}.off`, 'r+'))
      const [fhlog, fhoff] = await Promise.all(works)

      // rollback
      works = []
      const { hseq, hoff, hlen } = lock
      const off = await this._readOffset(fhoff)
      const { seq, size } = off ?? {}
      if (off !== null && hseq !== seq) {
        const end = size - 24n
        works.push(truncate(fhoff, end))
      }

      // rollback
      const start = max(hoff, 0n)
      const end = max(start + hlen, 0n)
      ...
      // reset lock
      await this._writeLock(fhlock, null, null, null)
        .then(() => sync(fhlock))
        .then(() => sync(this.dir))
  ...
  async start() {
    ...
    return this._readOffset().then((ok) => {
      ...
      return this._readHead(off).then((head) => {
        // todo: could test offset len against head len
        this.offset = off
        this.head = head
        this.hlen = BigInt(head.byteLength)
  ...
  async append(data, seq=null) {
    ...
    // state to lock
    await this._writeLock(BigInt(this.seq), this.offset, this.hlen)
    ...
    // reset lock
    await this._writeLock(null, null, null)
      .then(() => sync(this.fhlock))
  ...
  iter(seq='0', opts={}) {
    const { path: name } = this
    seq = parseBigInt(seq, new Error(`${name} seq must be string number`))
    if (seq < 0n) { throw new Error(`${name} seq must not be >= 0`) }
    if (!this._open) { throw new Error(`${name} is not open`) }
...
class FsIterator {
  ...
  // todo: wrap errors with iter name
  async *lazy() {
    const { log, iterStepSize } = this
    let next = this.seq
    const last = BigInt(log.seq)
    const lastLen = log.hlen

    while (next <= last) {
      const works = []
      if (!this._open) { throw new Error(`${this.name} is not open`) }
      let buf = await log._readOffsets(next, iterStepSize)
      if (!this._open) { throw new Error(`${this.name} is not open`) }

      console.log('ONE', next, last)
      for (let b = 0; b < buf.byteLength; b += 24) {
        ...
        console.log('TWO', seq, off, len)
        if (seq !== next) { throw new Error(`${this.name} seq out of order in read offsets loop`) }
        works.push([off, len])
        next = seq + 1n
        if (next > last) { break }
      }
      ...
      buf = await log._readLog(begin, end - begin)
      if (!this._open) { throw new Error(`${this.name} is not open`) }
      console.log('THREE', begin, end, end - begin)

      for (const work of works) {
        let [off, len] = work
        off = Number(off - begin)
        len = Number(len)
        console.log('FOUR', off, off + len)
