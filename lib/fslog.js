...
class FsLog {
  ...
  async _appendOffset(seq, off, len) {
    ...
    const buf = enc.allocMeta()
    await enc.encodeMeta(buf, 0, seq, off, len)
    const write = async () => {
  ...
  async _appendOffsets(offs) {
    const { fhoff: fh, encoder: enc } = this
    const buf = enc.allocMeta(offs.length)
    let o = 0
    for (const off of offs) {
      await enc.encodeMeta(buf, o, off.seq, off.off, off.len)
      o += enc.metaLen
    }
    const write = async () => {
      const { bytesWritten } = await fh.write(buf).catch((err) => {throw new Error(`${fh.name} write error - ${err.message}`)})
  ...
  async _readOffset() {
    ...
    const meta = await enc.decodeMeta(buf, 0)
  ...
  _appendLog(data) {
    ...
    const write = async () => {
      const buf = await enc.encodeBody(data)
      const { bytesWritten } = await fh.write(buf).catch((err) => {throw new Error(`${fh.name} write error - ${err.message}`)})
  ...
  _appendLogs(data) {
    ...
    const write = async () => {
      const works = data.map((buf) => enc.encodeBody(buf))
  ...
  async truncate(seq='-1') {
    ...
    const olen = BigInt(enc.metaLen) * (1n + seq)
    const llen = off + hlen
    await this._writeLock(olen, llen)
      .then(() => sync(this.fhlock))
