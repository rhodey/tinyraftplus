...
class FsLog {
  ...
  async append(data, seq=null) {
    ...
    const [_, lenn] = await Promise.all(works)
  ...
  async truncate(seq='-1') {
    ...
    if (seq === -1n) {
      await truncate(this.fhlog, 0n)
      await sync(this.fhlog)
      await truncate(this.fhoff, 0n)
      await sync(this.fhoff)
      this.offset = -1n
      this.olen = -1n
      this.hlen = -1n
      this.head = null
    ...
    if (buf.byteLength !== 24) { throw new Error(`${name} error reading offset for truncate ${seq}`) }
    const off = buf.readBigInt64BE(8)
    const hlen = buf.readBigInt64BE(16)

    const llen = off + hlen
    await truncate(this.fhlog, llen)
    await sync(this.fhlog)

    const olen = 24n * (1n + seq)
    await truncate(this.fhoff, olen)
    await sync(this.fhoff)
