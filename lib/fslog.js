...
// todo: use timeouts
// todo: name and ext in all errors
...
class FsLog {
  ...
  async _handleLock() {
    ...
    try {
      ...
      // rollback
      works = []
      works.push(sync(fhlog, 'log'))
      works.push(sync(fhoff, 'off'))
      await Promise.all(works)

      // reset lock
      await this._writeOffset(fhlock, 'lock', null)
        .then(() => sync(fhlock, 'lock'))

    } finally {
      await Promise.all(handles.map((h) => close(h[0], h[1])))
    }
  }
  ...
  async append(data, seq=null) {
    ...
    await this._writeOffset(this.fhlock, 'lock', offset)
      .then(() => sync(this.fhlock, 'lock'))

    // write work
    let works = []
    works.push(this._writeLog(data))
    works.push(this._writeOffset(this.fhoff, 'off', offset))
    ...
    // reset lock
    await this._writeOffset(this.fhlock, 'lock', null)
      .then(() => sync(this.fhlock, 'lock'))
