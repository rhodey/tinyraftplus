...
class FsLog {
  ...
  async _readOffset(fh) {
    ...
    const { bytesRead } = await fh.read(buf, 0, 16, Number(pos)).catch((err) => {throw new Error(`${fh.name} read error - ${err.message}`)})
  ...
  async _readHead(arg1, arg2) {
    ...
    const { bytesRead } = await fh.read(buf, 0, len, Number(offset)).catch((err) => {throw new Error(`${fh.name} read error - ${err.message}`)})
  ...
  async _handleRollback() {
    ...
    try {
      ...
      if (off !== null && hseq !== off.seq) {
        const { size } = off
        const end = size - 16n
        works.push(truncate(fhoff, Number(end)))
      }

      // rollback
      const start = Math.max(Number(hoff), 0)
      const end = Math.max(start + Number(hlen), 0)
